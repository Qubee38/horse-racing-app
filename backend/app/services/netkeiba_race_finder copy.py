# backend/app/services/netkeiba_race_finder.py (Selenium対応版)
import re
import time
import logging
...
class NetkeibaRaceFinder:
    ...
    def _get_races_with_selenium(self, formatted_date: str) -> List[Dict]:
        ...
        try:
            ...
            # レースIDを抽出
            races = self._extract_races_from_selenium_page(soup, formatted_date)
    ...
    def _extract_races_from_selenium_page(self, soup: BeautifulSoup, formatted_date: str) -> List[Dict]:
        """Seleniumで取得したページからレース情報を抽出"""
        races = []
        
        try:
            # 提供されたコードのセレクタを使用
            race_links = soup.select('.RaceList_DataItem > a:first-of-type')
            ...
            for link in race_links:
                try:
                    ...
                    # レース番号を抽出
                    race_number_match = re.search(r'(\d+)R', link_text)
                    race_number = int(race_number_match.group(1)) if race_number_match else 0
                    
                    # レース名を抽出
                    race_name = re.sub(r'^\d+R\s*', '', link_text).strip()
                    if not race_name:
                    ...
                    logger.warning(f"Failed to parse race link: {e}")
                    continue
            ...
            logger.error(f"Failed to extract races from Selenium page: {e}")
    ...
    def _get_races_with_requests(self, formatted_date: str) -> List[Dict]:
        ...
        try:
            # 複数のURLパターンを試行
            url_patterns = [
                f"{self.base_url}/top/race_list.html?kaisai_date={formatted_date}",
                f"{self.base_url}/top/race_list_sub.html?kaisai_date={formatted_date}",
            ]
            ...
            for url in url_patterns:
                try:
                    ...
                    # 複数のセレクタパターンを試行
                    selectors = [
                        '.RaceList_DataItem > a:first-of-type',
                        'a[href*="race_id="]',
                        '.race_kaisai_info a[href*="shutuba"]'
                    ]
                    
                    for selector in selectors:
                        race_links = soup.select(selector)
                        if race_links:
                            logger.info(f"Found {len(race_links)} links with selector: {selector}")
                            races = self._extract_races_from_requests_page(soup, formatted_date, selector)
                            if races:
                                return races
    ...
    def _extract_races_from_requests_page(self, soup: BeautifulSoup, formatted_date: str, selector: str) -> List[Dict]:
        """requestsで取得したページからレース情報を抽出"""
        races = []
        ...
        try:
            ...
            for link in race_links:
                try:
                    href = link.get('href', '')
                    if not href or 'race_id=' not in href:
                        continue
            ...
            logger.error(f"Failed to extract races from requests page: {e}")
    ...
    def _extract_venue_from_race_id(self, race_id: str) -> str:
        ...
        venue_map = {
            ...
            '09': '阪神', '10': '小倉', '11': '門別', '12': '盛岡',
            '13': '水沢', '14': '浦和', '15': '船橋', '16': '大井',
            '17': '川崎', '18': '金沢', '19': '笠松', '20': '名古屋',
            '21': '園田', '22': '姫路', '23': '益田', '24': '佐賀',
            '25': '荒尾'
        }
        
        return venue_map.get(place_code, "未定")
